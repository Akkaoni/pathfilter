...
def open_settings_window(main_window):
    ...
    btn_find_folder = tk.Button(settings_window, text="Обзор", command=choose_folder)
    btn_find_folder.pack(pady=10)

    btn_set_default = tk.Button(settings_window, text="Сброс настроек", command=set_default_settings)
    btn_set_default.pack(pady=10)
    
    btn_save = tk.Button(settings_window, text="Сохранить", command=save_settings, state='disabled')
    btn_save.pack(pady=10)

    btn_cancel = tk.Button(settings_window, text="Отмена", command=cancel)
